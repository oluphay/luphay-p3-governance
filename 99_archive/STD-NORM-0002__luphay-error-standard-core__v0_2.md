...
- §10.5 updated: INVARIANT and TRACEABILITY removed from families with no registry entry; STATE and SECURITY remain.
- `STD-SPEC-0006` added to Related Artifacts (§11).
...
| **Conditionally accepted** | Code enters the registry pending petitioning standard reaching ACTIVE status; implementations may use the code immediately |
...
The authoritative code catalog is maintained in `STD-REFR-0008__les-seed-catalog`. This section contains the current normative registry summary. Entries at **SEED** stage are pending formal registry lock by the steward. Entries at **CONDITIONAL** stage were accepted via petition pending the petitioning standard reaching ACTIVE status.

Severity values are `UNASSIGNED` pending approval of the LES severity framework.

### 10.1 Registry — Domain 1: Structural / Artifact

| Code | Family | Category | Object | Agent behavior | Entry stage | Petitioning standard |
|---|---|---|---|---|---|---|
| `LES__IDENTITY__MISMATCH__ARTIFACT-ID` | IDENTITY | MISMATCH | ARTIFACT-ID | REJECT | SEED | — |
| `LES__IDENTITY__INVALID__ARTIFACT-ID` | IDENTITY | INVALID | ARTIFACT-ID | REJECT | CONDITIONAL | STD-SPEC-0006 |
| `LES__IDENTITY__DUPLICATE__ARTIFACT-ID` | IDENTITY | DUPLICATE | ARTIFACT-ID | REJECT | CONDITIONAL | STD-SPEC-0006 |
...
| `LES__LIFECYCLE__PREMATURE__STATUS-TRANSITION` | LIFECYCLE | PREMATURE | STATUS-TRANSITION | REJECT | CONDITIONAL | STD-SPEC-0006 |
| `LES__LIFECYCLE__INVALID__STATUS-TRANSITION` | LIFECYCLE | INVALID | STATUS-TRANSITION | REJECT | CONDITIONAL | STD-SPEC-0006 |
| `LES__INVARIANT__VIOLATION__HARD-CONSTRAINT` | INVARIANT | VIOLATION | HARD-CONSTRAINT | REJECT | CONDITIONAL | STD-SPEC-0006 |
| `LES__INVARIANT__VIOLATION__DESIGN-INVARIANT` | INVARIANT | VIOLATION | DESIGN-INVARIANT | REJECT | CONDITIONAL | STD-SPEC-0006 |
| `LES__TRACEABILITY__MISSING__CONSUMER-LINK` | TRACEABILITY | MISSING | CONSUMER-LINK | ADVISORY; BLOCKING for ACTIVE/FROZEN in Assured mode | CONDITIONAL | STD-SPEC-0006 |
| `LES__TRACEABILITY__MISSING__OUTPUT-LINK` | TRACEABILITY | MISSING | OUTPUT-LINK | ADVISORY; BLOCKING for ACTIVE/FROZEN in Assured mode | CONDITIONAL | STD-SPEC-0006 |
| `LES__TRACEABILITY__INCOMPLETE__CONFORMANCE-CLAIM` | TRACEABILITY | INCOMPLETE | CONFORMANCE-CLAIM | REJECT for FULL claims; ADVISORY for PARTIAL | CONDITIONAL | STD-SPEC-0006 |
| `LES__TRACEABILITY__BROKEN__DEPENDENCY-LINK` | TRACEABILITY | BROKEN | DEPENDENCY-LINK | BLOCKING for graph compilation; ADVISORY for isolated validation | CONDITIONAL | STD-SPEC-0006 |
| `LES__TRACEABILITY__UNLINKED__ARTIFACT` | TRACEABILITY | UNLINKED | ARTIFACT | ADVISORY | CONDITIONAL | STD-SPEC-0006 |
...
| `LES__INTEGRITY__MISSING__CONTENT-HASH` | INTEGRITY | MISSING | CONTENT-HASH | BLOCKING for lifecycle promotion; ADVISORY in DRAFT | CONDITIONAL | STD-SPEC-0006 |
...
The following families are valid and active in the taxonomy but have no seed or conditional code. They are available for consuming standard petitions.
...
| `STD-SPEC-0006` | Luphay Artifact Metadata Standard (LAMS) v2.0.0 — petitioning standard for 12 codes accepted in v0.2 |
...
| `TASK-CHNG-0001` | LES petition — LAMS v2.0.0 error code registration (12 codes) |
